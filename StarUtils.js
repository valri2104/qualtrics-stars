...
window.StarUtils = (function () {
  ...
  /* ---------------------------------------------------------------------
     API PÚBLICA: addLabels → añade texto lateral
  --------------------------------------------------------------------- */

  function addLabels(questionIdOrElem, options) {
    var leftText  = options.leftText  || 'Muy insatisfecho';
    var rightText = options.rightText || 'Muy satisfecho';
    ...
    setTimeout(function () {
      ...
      containers.forEach(function(c) {
        ...
        if (c.parentNode.classList.contains('star-flex-wrapper')) return;

        var wrapper = document.createElement('div');
        wrapper.className = 'star-flex-wrapper';
